...
## 프록시
실제 엔티티 객체 대신에 데이터베이스 조회를 지연할 수 있는 가짜 객체가 필요한데 이것을 프록시 객체라 함
...
- orphanRemoval = true // 부모와 연관관계가 끊길때 삭제
- CASCADE.REMOVE // 부모 엔티티가 삭제될 떄 같이 삭제
...
- 즉시로딩시 발생
- 객체를 데이터베이스에서 불러올 때 1개읠 쿼리가 아닌 연관관계 객체를 불러오기 위한 N개의 쿼리가 생성되는 문제

  - 해결 방법
    - join fetch 사용
      조회시 바로 가져오고 싶은 Entity 필드 지정 / 중복된거 distinct
